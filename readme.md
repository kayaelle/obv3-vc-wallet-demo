...
Why use this fork of the wallet for this demo? This fork contains some code changes that will display a credential like an Open Badge. Also, this fork has the demo app registered with a DID:key for the Issuer and the url localhost:5000 so that it knows who this issuer is. 
...
* This demo isn't using an Open Badge context yet but coimng soo. In the short-term, Open Badge properties are linked to directly in the context of the badge data.
...
* Wallet may not accept SVG. Badge image in Badgr badgeclass is svg. See if wallet will accept SVGs because svg is a valid badge image format.
